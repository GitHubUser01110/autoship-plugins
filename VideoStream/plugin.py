...
class VideoFrameHandler:
        ...
        for subscriber in subscribers:
            try:
                if subscriber not in self.frame_queues:
                    ...
                    task = asyncio.create_task(self._send_worker(subscriber, camera_id))
    ...
    async def _send_worker(self, subscriber: websockets.WebSocketServerProtocol, camera_id: str):
        queue = self.frame_queues[subscriber]
        last_send_time = 0
...
class VideoStreamPlugin(Plugin):
    ...
    def __init__(self, plugin_id: str, plugin_manager):
        ...
        # 分片重组缓存
        self._fragment_buffer: Dict[str, Dict[int, Dict[str, Any]]] = {}
        self._fragment_lock = threading.Lock()
        self._fragment_timeout = 2.0
    ...
    def _run_fragment_cleanup(self):
        """定期清理过期的分片缓存"""
        try:
            self.log_info("分片清理线程已启动")
            while self._running.is_set():
                ...
                with self._fragment_lock:
                    ...
                    for camera_id, frames in self._fragment_buffer.items():
                        frames_to_remove = []
                        
                        for frame_seq, frame_data in frames.items():
                            if current_time - frame_data['first_fragment_time'] > self._fragment_timeout:
                                frames_to_remove.append(frame_seq)
                        
                        for frame_seq in frames_to_remove:
                            del frames[frame_seq]
                            self.log_warning(f"清理超时分片: {camera_id} 帧{frame_seq}")
                        
                        if not frames:
                            cameras_to_remove.append(camera_id)
                    
                    for camera_id in cameras_to_remove:
                        del self._fragment_buffer[camera_id]
        except Exception as e:
            if self._running.is_set():
                self.log_error(f"分片清理线程错误: {e}\n{traceback.format_exc()}")
        finally:
            self.log_info("分片清理线程已停止")
        ...
        try:
            ...
            with self._fragment_lock:
                ...
                if len(frame_data['fragments']) == frame_data['total_fragments']:
                    ...
                    if complete_frame:
                        self._process_complete_frame(complete_frame, camera_id, frame_seq, timestamp)
        except Exception as e:
    ...
    def _process_complete_frame(self, complete_data: bytes, camera_id: str, 
        ...
        """处理重组后的完整帧"""
        try:
            offset = 8 + len(camera_id.encode('utf-8')) + 12
    ...
    def _get_performance_recommendations(self, video_stats: Dict) -> list:
        ...
        if video_stats['avg_frame_size'] > 100 * 1024:
            recommendations.append({
                'level': 'info',
